...
//const { registerUser, loginUser } = require('./mysqlConnectionTest'); 
...
app.post('/user/register', async (req, resp) => {
    ...
    try {
        ...
        resp.json({ status: 200, msg: 'register successfully' });
    } catch (error) {
...
app.post('/user/login', async (req, resp) => {
    ...
    try {
        ...
        if (rows.length == 0) {
            resp.json({ status: 500, data: "account or password wrong" });
            return;
        }
...
// Export the app for use in your test file
//module.exports = app;
...
console.log(__dirname);  // This will log the absolute path of the directory where server.js is located
